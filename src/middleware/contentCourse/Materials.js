...
const Materials = () => {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (offsetTop <= 0) {
        ...
        videoContainer.style.transform = 'scale(1.05)'; // Apply the scaling transform for transition
        videoContainer.style.transition = 'all 1s ease-in-out'
      } else {
        videoContainer.style.width = 'auto';
        videoContainer.style.position = 'static';
        videoContainer.style.zIndex = '1';
        videoContainer.style.transform = 'scale(1)'; // Reset the transform
        videoContainer.style.transition = '1s ease-in-out'
      }
    };
  ...
  const getNextTab = () => {
    ...
    localStorage.setItem('lastVisitedLesson', currentTab.toString());
  };
          ...
          <Grid container spacing={3}>
            ...
            <Grid item xs={12} md={4} lg={3}>
              ...
              {currentTab === contentData.length - 1 ? (
                ...
                <Button variant="contained" color="primary" onClick={(event) => getNextModule(event, getNextMod)} sx={{ mt: 2 }} disabled={isVideoPlaying}>
                  Next module
                </Button>
                : (
                  <Button variant="contained" color="primary" onClick={getNextTab} sx={{ mt: 2 }} disabled={isVideoPlaying}>
